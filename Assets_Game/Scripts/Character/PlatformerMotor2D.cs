...
public class PlatformerMotor2D : MonoBehaviour
{
    ...
    private void SetLastJumpType()
    {
        ...
        else
        {
            ...
            if (PressingIntoLeftWall() &&
                (obstacle = _collidersUpAgainst[DIRECTION_LEFT].GetComponent<Wall>()) != null &&
                obstacle.enableWallJumps &&
                obstacle.overrideWallInteractions)
            {
                _jumping.lastValidJump = JumpState.JumpType.LeftWall;
                //Debug.Log("OVERRIDED LEFT WALL JUMP");
            }
            else if (PressingIntoRightWall() &&
                (obstacle = _collidersUpAgainst[DIRECTION_RIGHT].GetComponent<Wall>()) != null &&
                obstacle.enableWallJumps &&
                obstacle.overrideWallInteractions)
            {
                _jumping.lastValidJump = JumpState.JumpType.RightWall;
        ...
        if (_jumping.pressed)
        {
                ...
                (_isValidWallInteraction && PressingIntoLeftWall())))
            {
                ...
                if (_collidersUpAgainst[DIRECTION_LEFT])
                    obstacle = _collidersUpAgainst[DIRECTION_LEFT].GetComponent<Wall>();

                Vector2 wallJumpVector = _wallJumpVector;
                float localWallJumpMultiplier = wallJumpMultiplier;
                bool localEnableWallJumps = enableWallJumps;

                if (obstacle != null && obstacle.overrideWallInteractions)
                {
                    wallJumpVector = Quaternion.AngleAxis(obstacle.wallJumpAngle, Vector3.forward) * Vector3.right;
                    localWallJumpMultiplier = obstacle.wallJumpMultiplier;
                    localEnableWallJumps = obstacle.enableWallJumps;
                }

                if (localEnableWallJumps)
                {
                    // If jump was pressed as we or before we entered the wall then just jump away.
                    _velocity = wallJumpVector * CalculateSpeedNeeded(_jumping.height) * localWallJumpMultiplier;
                ...
                (_isValidWallInteraction && PressingIntoRightWall())))
            {
                ...
                if (_collidersUpAgainst[DIRECTION_RIGHT])
                    obstacle = _collidersUpAgainst[DIRECTION_RIGHT].GetComponent<Wall>();
    ...
    private void HandlePreWallInteraction()
    {
        ...
        if (_isValidWallInteraction && _velocity.y <= 0 && (PressingIntoLeftWall() || PressingIntoRightWall()) && _wallInfo.canHangAgain)
        {
            if ((PressingIntoLeftWall() &&
                ...
                obstacle.overrideWallInteractions))
            {
                _wallInfo.stickFrames = GetFrameCount(obstacle.wallSticksDuration);
                _velocity = Vector2.zero;
                _wallInfo.canHangAgain = false;
                ChangeState(MotorState.WallSticking);
                return;
            }
            else if (enableWallSticks)
            {
    ...
    private CollidedSurface CheckSurroundings(bool forceCheck)
    {
            ...
            (HasFlag(CollidedSurface.Ground) && IsJumping()))
        {
            ...
            // 임시, 하드코딩 돼있음
            if ((surfaces & CollidedSurface.Ground) != CollidedSurface.None)
            {
                Spring spring = _collidersUpAgainst[DIRECTION_DOWN].GetComponent<Spring>();

                if (spring != null)
                {
                    Jump(spring.jumpHeight);
                }
                else
                {
                    RaycastHit2D[] grounds = new RaycastHit2D[2];
                    ...
                    {
                        spring = grounds[i].collider.GetComponent<Spring>();
                        if (spring != null)
                        {
                            Jump(spring.jumpHeight);
                            break;
