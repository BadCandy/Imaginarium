...
public class PlayerController2D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (CnInputManager.GetButtonDown(PC2D.Input.JUMP))
        {
            _motor.Jump();
            _motor.DisableRestrictedArea();
        }

        _motor.jumpingHeld = CnInputManager.GetButton(PC2D.Input.JUMP);

        // XY freedom movement
        if (_motor.motorState == PlatformerMotor2D.MotorState.FreedomState)
        {
            _motor.normalizedXMovement = CnInputManager.GetAxis(PC2D.Input.HORIZONTAL);
            _motor.normalizedYMovement = CnInputManager.GetAxis(PC2D.Input.VERTICAL);

            return; // do nothing more
        }

        // X axis movement
        if (Mathf.Abs(CnInputManager.GetAxis(PC2D.Input.HORIZONTAL)) > PC2D.Globals.INPUT_THRESHOLD)
        {
            _motor.normalizedXMovement = CnInputManager.GetAxis(PC2D.Input.HORIZONTAL);
        }
        else
        ...
        if (CnInputManager.GetAxis(PC2D.Input.VERTICAL) != 0)
        {
            bool up_pressed = CnInputManager.GetAxis(PC2D.Input.VERTICAL) > 0;
        ...
        else if (CnInputManager.GetAxis(PC2D.Input.VERTICAL) < -PC2D.Globals.FAST_FALL_THRESHOLD)
        {
            _motor.fallFast = false;
        }

        if (CnInputManager.GetButtonDown(PC2D.Input.DASH))
        {
            _motor.Dash();
        }

        if (CnInputManager.GetButtonDown(PC2D.Input.ACTION))
        {
            _ignoreActionKey = _player.Action();
        }

        if (/*CnInputManager.GetButton(PC2D.Input.ACTION) && */!_ignoreActionKey && !_player.IsHoldingBlock)
        {
            _ignoreActionKey = _player.Action();
        }

        if (CnInputManager.GetButtonUp(PC2D.Input.ACTION))
        {
            _ignoreActionKey = true;
        }
    }
}
